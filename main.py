...
# Number of messages fetched from the discord history, not guaranteed to be included in prompt
DISCORD_HISTORY_DEPTH = 40
...
TRIGGER_ON_QUESTION = False  # Trigger when a message contains a ?
    ...
    while numMessages < MAX_MESSAGES_CONSIDERED_FOR_PROMPT and len(totalMessage) < MAX_TOTAL_PROMPT_CHARACTERS:
        if curMessage.reference is not None:
            ...
            if msgBefore.author.id != curMessage.author.id and msgBefore.author != client.user:
                print("author of message before does not match {} != {}".format(
                    msgBefore.author.id, curMessage.author.id))
                break
...
async def get_message_to_reply_to(message: discord.Message):
    if time.time() - LAST_REQUEST < RATELIMIT_GENERAL:
        return None
    if TRIGGER_ON_QUESTION and is_ordinary_question(message):
        return message
...
class MyClient(discord.Client):
    async def on_ready(self):
        ...
        game = discord.Game("@ me or ask a question!")
